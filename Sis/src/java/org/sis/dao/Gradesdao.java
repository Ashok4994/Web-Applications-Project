...
public class Gradesdao {
    ...
    public float calsgpa(float cng, float cdg, float wtg, float lig, float cgg, float peg, float wtlabg, float lilabg, float compviva1g,String sid) {
        ...
        try {
            pstmt = cp.prepareStatement("select  subname,credits from adminlog where reg=11 and sem=6;");
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                ...
                if ("cn".equals(name)) {
                    ...
                    peli = c;
                    peui = peg * peli;
                } else if ("wtlab".equals(name)) {
                    wtlabli = c;
                    wtlabui = wtlabg * wtlabli;
                } else if ("lilab".equals(name)) {
                    lilabli = c;
                    lilabui = lilabg * lilabli;
                } else if ("cmpviva1".equals(name)) {
                    compviva1li = c;
...
//            System.out.println("sgpa=" + sgpa);
     ...
     int j = 0;
        ...
        try {
            pstmt3 = cp.prepareStatement("update threeone set Tou=?,Lou=? where sid=?");
            pstmt3.setFloat(1, u);
            pstmt3.setFloat(2, l);
            pstmt3.setString(3, sid);
            j = pstmt3.executeUpdate();
            pstmt1 = cp.prepareStatement("select Touc,Louc FROM fifth where sid=?");
            ...
            pstmt2 = cp.prepareStatement(" update threeone set Touc=?,Louc=?,Cgpa=? where sid=? ");
    ...
    public int updategMarks(String scn, String scd, String swt, String sli, String scg, String spe, String swtlab, String slilab, String scompviva1, String sgpa, String nbl, String sid) {
        ...
        PreparedStatement pstmt;
        try {
            pstmt = cp.prepareStatement(" update threeone set gcn=?,gcd=?,gwt=?,gli=?,gcg=?,gpe=?, gwtlab=?, glilab=?, gcompviva1=?, sgpa=?, Nbl=? where sid=? ");
