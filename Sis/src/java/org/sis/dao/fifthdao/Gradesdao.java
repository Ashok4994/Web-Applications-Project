...
public class Gradesdao {
    ...
    public float calsgpa(float cng, float cdg, float wtg, float lig, float cgg, float peg, float wtlabg, float lilabg, float compviva1g, String sid) {
        ...
        try {
            pstmt = cp.prepareStatement("SELECT subname,credits FROM adminlog where reg=11 and sem=5;");
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                ...
                } else if ("atfl".equals(name)) {
                    cdli = c;
                    cdui = cdg * cdli;
                } else if ("ooad".equals(name)) {
                    wtli = c;
                    wtui = wtg * wtli;
                } else if ("ppl".equals(name)) {
                    lili = c;
                    liui = lig * lili;
                } else if ("mpi".equals(name)) {
                    cgli = c;
                    cgui = cgg * cgli;
                } else if ("stm".equals(name)) {
                    peli = c;
                    peui = peg * peli;
                } else if ("ooadlab".equals(name)) {
                    wtlabli = c;
                    wtlabui = wtlabg * wtlabli;
                } else if ("mpilab".equals(name)) {
                    lilabli = c;
                    lilabui = lilabg * lilabli;
                } else if ("seminar2".equals(name)) {
                    compviva1li = c;
...
//            System.out.println("sgpa=" + sgpa);
    ...
    public int updatelu(float u, float l, String sid) {
        ...
        try {
            pstmt3 = cp.prepareStatement("update fifth set Tou=?,Lou=? where sid=?");
            pstmt3.setFloat(1, u);
            pstmt3.setFloat(2, l);
            pstmt3.setString(3, sid);
            j = pstmt3.executeUpdate();
            pstmt1 = cp.prepareStatement("SELECT Touc,Louc FROM fourth where sid=?");
            ...
            pstmt2 = cp.prepareStatement(" update fifth set Touc=?,Louc=?,Cgpa=? where sid=? ");
    ...
    public int updategMarks(String scn, String scd, String swt, String sli, String scg, String spe, String swtlab, String slilab, String scompviva1, String sgpa, String nbl, String sid) {
        ...
        PreparedStatement pstmt;
        try {
            pstmt = cp.prepareStatement(" update fifth set gcn=?,gcd=?,gwt=?,gli=?,gcg=?,gpe=?, gwtlab=?, glilab=?, gcompviva1=?, sgpa=?, Nbl=? where sid=? ");
