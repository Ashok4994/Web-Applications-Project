...
public class Gradesdao {
    ...
    public ArrayList studentUtils() {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String dtpa = rs.getString("adtp");
                if (Integer.parseInt(dtpa) > 90) {
                    cga = a;
                } else if (Integer.parseInt(dtpa) > 85 && Integer.parseInt(dtpa) <= 90) {
                    dtpa = b;
                } else if (Integer.parseInt(cga) > 80 && Integer.parseInt(cga) <= 85) {
                    dtpa = c;
                } else if (Integer.parseInt(dtpa) > 75 && Integer.parseInt(dtpa) <= 80) {
                    dtpa = d;
                } else {
                    dtpa = e;
                }
                ...
                float dtpg = Integer.parseInt(dtpa) + Integer.parseInt(dtpm) + Integer.parseInt(dtps);
                if (dtpg >= 90) {
                    dtpg = (float) 4.00;
                } else if (dtpg >= 85 && dtpg < 90) {
                    dtpg = (float) 3.67;
                } else if (dtpg >= 80 && dtpg < 85) {
                    dtpg = (float) 3.33;
                } else if (dtpg >= 75 && dtpg < 80) {
                    dtpg = (float) 3.00;
                } else if (dtpg >= 70 && dtpg < 75) {
                    dtpg = (float) 2.67;
                } else if (dtpg >= 65 && dtpg < 70) {
                    dtpg = (float) 2.33;
                } else if (dtpg >= 60 && dtpg < 65) {
                    dtpg = (float) 2.00;
                } else if (dtpg >= 55 && dtpg < 60) {
                    dtpg = (float) 1.67;
                } else if (dtpg >= 50 && dtpg < 55) {
                    dtpg = (float) 1.33;
                } else if (dtpg >= 40 && dtpg < 50) {
                    dtpg = (float) 1.00;
                } else {
                    dtpg = 0;
                }
                ...
                mp.put("sdtp", dtpl);
                mp.put("nbl", nbl);
                float sgpa = calsgpa(cng, cdg, wtg, lig, cgg, peg, wtlabg, lilabg, compviva1g,dtpg, sid);
                mp.put("sgpa", sgpa);
    ...
    public float calsgpa(float cng, float cdg, float wtg, float lig, float cgg, float peg, float wtlabg, float lilabg, float compviva1g,float dtpg, String sid) {
        String name = null;
        ...
        try {
            pstmt = cp.prepareStatement("SELECT subname,credits FROM adminlog where reg=11 and sem=7;");
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                ...
                } else if ("dmdw".equals(name)) {
                    cdli = c;
                    cdui = cdg * cdli;
                } else if ("mc".equals(name)) {
                    wtli = c;
                    wtui = wtg * wtli;
                } else if ("spm".equals(name)) {
                    lili = c;
                    liui = lig * lili;
                } else if ("aca".equals(name)) {
                    cgli = c;
                    cgui = cgg * cgli;
                } else if ("ai".equals(name)) {
                    peli = c;
                    peui = peg * peli;
                } else if ("mclab".equals(name)) {
                    wtlabli = c;
                    wtlabui = wtlabg * wtlabli;
                } else if ("stmlab".equals(name)) {
                    lilabli = c;
                    lilabui = lilabg * lilabli;
                } else if ("internship".equals(name)) {
                    compviva1li = c;
                    compviva1ui = compviva1g * compviva1li;
                }
                else if ("termpaper".equals(name)) {
                    dtpli = c;
                    dtpui = dtpg * dtpli;
                }
            }
            u = cnui + cdui + wtui + liui + cgui + peui + wtlabui + lilabui + compviva1ui+dtpui;

            l = cnli + cdli + wtli + lili + cgli + peli + wtlabli + lilabli + compviva1li+dtpli;
    ...
    public int updatelu(float u, float l, String sid) {
        ...
        try {
            pstmt3 = cp.prepareStatement("update seventh set Tou=?,Lou=? where sid=?");
            pstmt3.setFloat(1, u);
            pstmt3.setFloat(2, l);
            pstmt3.setString(3, sid);
            j = pstmt3.executeUpdate();
            pstmt1 = cp.prepareStatement("SELECT Touc,Louc FROM threeone where sid=?");
            ...
            pstmt2 = cp.prepareStatement(" update seventh set Touc=?,Louc=?,Cgpa=? where sid=? ");
    ...
    public int updategMarks(String scn, String scd, String swt, String sli, String scg, String spe, String swtlab, String slilab, String scompviva1,String sdtp, String sgpa, String nbl, String sid) {
        int j = 0;
        Databaseconn db = new Databaseconn();
        Connection cp = db.getConnection();
        db = null;
        PreparedStatement pstmt;
        try {
            pstmt = cp.prepareStatement(" update seventh set gcn=?,gcd=?,gwt=?,gli=?,gcg=?,gpe=?, gwtlab=?, glilab=?, gcompviva1=?, gdtp=?, sgpa=?, Nbl=? where sid=? ");
            ...
            pstmt.setString(11, sgpa);
            pstmt.setString(12, nbl);
            pstmt.setString(13, sid);
