...
import org.sis.dao.thirddao.StdmarksEntryAction;
...
public class Gradesdao {
    ...
    public float calsgpa(float cng, float cdg, float wtg, float lig, float cgg, float peg, float wtlabg, float lilabg, float compviva1g, String sid) {
        ...
        try {
            pstmt = cp.prepareStatement("SELECT subname,credits FROM adminlog where reg=11 and sem=2;");
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                ...
                if ("am2".equals(name)) {
                    cnli = c;
                    cnui = cng * cnli;
                } else if ("eng2".equals(name)) {
                    cdli = c;
                    cdui = cdg * cdli;
                } else if ("chem".equals(name)) {
                    wtli = c;
                    wtui = wtg * wtli;
                } else if ("ds".equals(name)) {
                    lili = c;
                    liui = lig * lili;
                } else if ("phy".equals(name)) {
                    cgli = c;
                    cgui = cgg * cgli;
                } else if ("atcd".equals(name)) {
                    peli = c;
                    peui = peg * peli;
                } else if ("phychelab".equals(name)) {
                    wtlabli = c;
                    wtlabui = wtlabg * wtlabli;
                } else if ("dslab".equals(name)) {
                    lilabli = c;
                    lilabui = lilabg * lilabli;
                } else if ("mprj1".equals(name)) {
                    compviva1li = c;
    ...
    public int updatelu(float u, float l, String sid) {
        ...
        try {
            pstmt3 = cp.prepareStatement("update second set Tou=?,Lou=? where sid=?");
            pstmt3.setFloat(1, u);
            pstmt3.setFloat(2, l);
            pstmt3.setString(3, sid);
            j = pstmt3.executeUpdate();
            pstmt1 = cp.prepareStatement("SELECT Tou,Lou FROM first where sid=?");
              pstmt1.setString(1, sid);
            ResultSet rs = pstmt1.executeQuery();
            while (rs.next()) {
                touf = rs.getFloat("Tou");
                louf = rs.getFloat("Lou");
            }
            touf = touf + tous;
            louf = lous + louf;
            float cgpa = (touf) / (louf);
            pstmt2 = cp.prepareStatement(" update second set Touc=?,Louc=?,Cgpa=? where sid=? ");
    ...
    public int updategMarks(String scn, String scd, String swt, String sli, String scg, String spe, String swtlab, String slilab, String scompviva1, String sgpa, String nbl, String sid) {
        ...
        PreparedStatement pstmt;
        try {
            pstmt = cp.prepareStatement(" update second set gcn=?,gcd=?,gwt=?,gli=?,gcg=?,gpe=?, gwtlab=?, glilab=?, gcompviva1=?, sgpa=?, Nbl=? where sid=? ");
