...
public class Gradesdao {
    ...
    public ArrayList studentUtils() {
        ...
        try {
            pstmt = cp.prepareStatement("select * from first");
            ...
            while (rs.next()) {
                ...
                float sgpa = calsgpa(cng, cdg, wtg, lig, cgg, peg, wtlabg, lilabg,sid);
    ...
    public float calsgpa(float cng, float cdg, float wtg, float lig, float cgg, float peg, float wtlabg, float lilabg, String sid) {
        String name = null;
        ...
        try {
            pstmt = cp.prepareStatement("select subname,credits FROM adminlog where reg=11 and sem=1;");
            ResultSet rs = pstmt.executeQuery();
            while (rs.next()) {
                ...
                } else if ("cprg".equals(name)) {
                    cdli = c;
                    cdui = cdg * cdli;
                } else if ("eng1".equals(name)) {
                    wtli = c;
                    wtui = wtg * wtli;
                } else if ("nm1".equals(name)) {
                    lili = c;
                    liui = lig * lili;
                } else if ("es".equals(name)) {
                    cgli = c;
                    cgui = cgg * cgli;
                } else if ("cplab".equals(name)) {
                    peli = c;
                    peui = peg * peli;
                } else if ("englab".equals(name)) {
                    wtlabli = c;
                    wtlabui = wtlabg * wtlabli;
                } else if ("engwrkshp".equals(name)) {
                    lilabli = c;
                    lilabui = lilabg * lilabli;
                }
            }
            u = cnui + cdui + wtui + liui + cgui + peui + wtlabui + lilabui ;
            //System.out.println("u=" + u);
            l = cnli + cdli + wtli + lili + cgli + peli + wtlabli + lilabli  ;
            ...
            //  System.out.println("u=" + sgpa);
//            System.out.println("l=" + sgpa);
//            System.out.println("sgpa=" + sgpa);
        } catch (SQLException ex) {
            Logger.getLogger(StdmarksEntryAction.class.getName()).log(Level.SEVERE, null, ex);
        }
     ...
     public int updatelu(float u,float l, String sid) {
        ...
        PreparedStatement pstmt;
        try {
            pstmt = cp.prepareStatement(" update first set Tou=?,Lou=? where sid=? ");
           pstmt.setFloat(1, u);
            pstmt.setFloat(2, l);
    ...
    public int updategMarks(String scn, String scd, String swt, String sli, String scg, String spe, String swtlab, String slilab,  String sgpa, String nbl, String sid) {
        int j = 0;
        Databaseconn db = new Databaseconn();
        Connection cp = db.getConnection();
        db = null;
        PreparedStatement pstmt;
        try {
            pstmt = cp.prepareStatement(" update first set gcn=?,gcd=?,gwt=?,gli=?,gcg=?,gpe=?, gwtlab=?, glilab=?, sgpa=?, Nbl=? where sid=? ");
            ...
            pstmt.setString(9, sgpa);
            pstmt.setString(10, nbl);
